...
public class EnemyScript : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        guessGrid = Enumerable.Repeat('o', 70).ToArray();
    }
    ...
    public List<int[]> PlaceEnemyShips()
    {
        ...
        int[] gridNumbers = Enumerable.Range(1, 70).ToArray();
        bool taken = true;
        foreach(int[] tileNumArray in enemyShips)
        {
            ...
            {
                taken = false;
                int shipNose = UnityEngine.Random.Range(0, 69);
    ...
    public void NPCTurn()
    {
        ...
        if(hitIndex.Count > 1)
        {
            ...
            {
                if(guessGrid[nextIndex] == 'm' || nextIndex > 70 || nextIndex < 0)
                {
                    diff *= -1;
                }
                nextIndex += diff;
        ...
        else if (hitIndex.Count == 1)
        {
            ...
            bool onGrid = possibleGuess > -1 && possibleGuess < 70;
            while((!onGrid || guessGrid[possibleGuess] != 'o') && closeTiles.Count > 0){
                ...
                onGrid = possibleGuess > -1 && possibleGuess < 70;
            }
            guess = possibleGuess;
        }
        else
        {
            int nextIndex = Random.Range(0, 70);
            while(guessGrid[nextIndex] != 'o') nextIndex = Random.Range(0, 70);
    ...
    public void SendAttackOn(int guess) {
    ...
    private int GuessAgainCheck(int nextIndex)
    {
        ...
        if (nextIndex + 1 < 70) nearGuess = guessGrid[nextIndex + 1] != 'o';
        if (!nearGuess && nextIndex - 1 > 0) nearGuess = guessGrid[nextIndex - 1] != 'o';
        if (!nearGuess && nextIndex + 10 < 70) nearGuess = guessGrid[nextIndex + 10] != 'o';
        if (!nearGuess && nextIndex - 10 > 0) nearGuess = guessGrid[nextIndex - 10] != 'o';
        if (edgeCase || nearGuess) newGuess = Random.Range(0, 70);
        while (guessGrid[newGuess] != 'o') newGuess = Random.Range(0, 70);
